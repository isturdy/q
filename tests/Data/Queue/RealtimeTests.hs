module Data.Queue.RealtimeTests (realtimeTests) where
...
realtimeTests = testGroup "Data.Queue.Realtime" [
    consistencyTests
  ]

consistencyTests = testGroup "Consistency" [
    testCase     "mempty"     $ isEmpty (mempty::RealtimeQueue Int) @?= True
  ...
  , testCase     "qrec"       $ qrec (count_if_less 10) 0
                                     (fromList [1]::RealtimeQueue Int) @?= 143
  ]

prop_empty l1 l2 = (l1++l2==[]) == isEmpty q
  where q = toQueue l1 l2 :: RealtimeQueue Int
...
prop_mappend l11 l12 l21 l22 = fromList (l11++l12++l21++l22) == q1 <> q2
  where q1 = toQueue l11 l12 :: RealtimeQueue Int
        q2 = toQueue l21 l22 :: RealtimeQueue Int
